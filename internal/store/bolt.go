...
func OpenBolt(path string) (*BoltStore, error) {
	...
	err = db.Update(func(tx *bolt.Tx) error {
		if _, e := tx.CreateBucketIfNotExists(bucketSubscribers); e != nil {
			return e
		}
		if _, e := tx.CreateBucketIfNotExists(bucketQueries); e != nil {
			return e
		}
		return nil
	})
...
func (s *BoltStore) UpsertSubscriber(sub Subscriber) error {
	...
	return s.db.Update(func(tx *bolt.Tx) error {
		bucket := tx.Bucket(bucketSubscribers)
...
func (s *BoltStore) AddQuery(q Query) (string, error) {
	...
	err = s.db.Update(func(tx *bolt.Tx) error {
		bucket := tx.Bucket(bucketQueries)
